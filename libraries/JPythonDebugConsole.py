"""Modified by John Farrell for the Minestar project.
"""
...
def popup(title, ls):
    # ls is locals
    import com.mincom.util.log.Logging
    c = Console()
    for (key, jo) in ls.items():
        c.locals[key] = jo
        if key == 'ObjServerInstance':
            c.locals['objserver'] = jo
            if jo.getClass().getName() == 'com.mincom.env.container.ObjServerExImpl':
                keys = gather(jo.classList.keySet().iterator())
                for k in keys:
                    obj = jo.classList.get(k)
                    fs = string.split(k)
                    c.locals[fs[-1]] = obj
    c.locals['gather'] = gather
    c.locals['fields'] = fields
    c.locals['logging'] = com.mincom.util.log.Logging
    c.locals['loggingConfig'] = c.locals['logging'].config.getMap()
    pane = swing.JScrollPane(c.textpane)
    swing.test(pane, size=(500,400), name=title)
    c.capturePythonOutput()
    c.textpane.requestFocus()
    c.newInput()
...
def fields(obj):
    ...
    while 1:
        ...
        for fd in fds:
            if not Modifier.isStatic(fd.getModifiers()):
                fs.append(fd)
