...
APP = "emptyDataStore"
ALL_DBS = "_MODELDB,_HISTORICALDB,_SUMMARYDB,_REPORTINGDB,_TEMPLATEDB,_PITMODELDB"
dbobject = databaseDifferentiator.returndbObject()

def emptyDataStore(dbName):
    # Assume _MODELDB, _HISTORICALDB, _TEMPLATEDB, _REPORTINGDB and _BOAUDITDB are configured in MineStar.properties
    ...
    try:
        progress.start(1000, "emptyDataStores")
        ...
        if dbName != "all":
            emptyDS = datastore.getDataStore(dbName)
            if emptyDS != None:
...
if __name__ == '__main__':
    ...
    if len(dbIds) == 1 and 'all' == dbIds[0].lower():
        print "Empty All Data Stores"
        dbIds = string.split(ALL_DBS, ',')

    for dbName in dbIds:
        print "calling emptyDataStores() with DbName = %s" % (dbName)
        emptyDataStore(dbName)
