...
class JavaByteArray(bytearray, JavaObject):
    ...
    def __repr__(self):
        ...
        return "{size:%r,data:%s}" % (len(self), hexlify(self))
...
class JavaObjectUnmarshaller(JavaObjectConstants):
    ...
    def do_object(self, parent=None, ident=0):
        ...
        if classdesc.flags & self.SC_SERIALIZABLE:
            ...
            while tempclass:
                ...
                for (field_type, field_name) in zip(tempclass.fields_types, tempclass.fields_names):
                    log_debug("%s : %s" % (field_name, field_type), ident + 2)
    ...
    def do_array(self, parent=None, ident=0):
        ...
        type_char_str = "<mumble>"
        if type_char == self.TYPE_OBJECT: type_char_str = "TYPE_OBJECT"
        if type_char == self.TYPE_ARRAY: type_char_str = "TYPE_ARRAY"
        if type_char == self.TYPE_BYTE: type_char_str = "TYPE_BYTE"
        log_debug("do_array() type_char=%s" % type_char_str)
...
class JavaObjectMarshaller(JavaObjectConstants):
    ...
    def writeObject(self, obj):
        ...
        if isinstance(obj, JavaByteArray):
            # Deserialized Java byte[]
            self.write_byte_array(obj)
    ...
    def _writeString(self, obj, use_reference=True):
        ...
        if use_reference and isinstance(obj, JavaString):
            try:
                ...
                self.references.append(obj)
                log_debug(
                    "*** Adding ref 0x%X for string: %s" % (len(self.references) - 1 + self.BASE_REFERENCE_IDX, obj))

                self._writeStruct(">H", 2, (len(string),))
                self.object_stream.write(string)
            else:
                # Write a reference to the previous type
                log_debug("*** Reusing ref 0x%X for string: %s" % (idx + self.BASE_REFERENCE_IDX, obj))
    ...
    def write_object(self, obj, parent=None):
        ...
        log_debug("write_object() class=%s" % cls.name)
        ...
        log_debug(
            "*** Adding ref 0x%X for object %s" % (len(self.references) - 1 + self.BASE_REFERENCE_IDX, obj))
        ...
        log_debug(" => Field names: %s" % all_names)
        log_debug(" => Field types: %s" % all_types)

        for (field_name, field_type) in zip(all_names, all_types):
            try:
                field_value = getattr(obj, field_name)
                log_debug("Writing field %s (%s): %s" % (field_name, field_type, field_value))
                self._write_value(field_type, field_value)
            except AttributeError as ex:
                log_error("No attribute {0} for object {1}\nDir: {2}".format(ex, repr(obj), dir(obj)))
                raise
    ...
    def write_classdesc(self, obj, parent=None):
        ...
        if obj not in self.references:
            ...
            self.references.append(obj)
            log_debug(
                "*** Adding ref 0x%X for classdesc %s" % (len(self.references) - 1 + self.BASE_REFERENCE_IDX, obj.name))
            ...
            for field_name, field_type in zip(obj.fields_names, obj.fields_types):
                log_debug("write_classdesc() field_name=%s, field_type=%s" % (field_name, field_type))
                ...
                if field_type[0] in (self.TYPE_OBJECT, self.TYPE_ARRAY):
                    try:
                        ...
                        log_debug(
                            "*** Adding ref 0x%X for field type %s" % (len(self.references) - 1 + self.BASE_REFERENCE_IDX, field_type))

                        self.write_string(field_type, False)
                    else:
                        # Write a reference to the previous type
                        log_debug("*** Reusing ref 0x%X for %s (%s)" % (idx + self.BASE_REFERENCE_IDX, field_type, field_name))
                        self.write_reference(idx)
    ...
    def write_byte_array(self, obj):
        """ 
        Writes a JavaByteArray
        
        :param obj a JavaByteArray object
        """

        log_debug("write_byte_array() obj=%s" % obj)
        ...
        self.references.append(obj)
        log_debug(
            "*** Adding ref 0x%X for array []" % (len(self.references) - 1 + self.BASE_REFERENCE_IDX))

        # Write the byte array to the stream.
        import ctypes
        args = [ctypes.c_byte(sb).value for sb in obj]
        encoding = struct.pack("%db" % len(obj), *args)
        
        self.object_stream.write(encoding)
    ...
    def _write_value(self, field_type, value):
        ...
        if field_type == self.TYPE_BOOLEAN:
            ...
            elif isinstance(value, JavaByteArray):
