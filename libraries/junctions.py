# Copied from: http://stackoverflow.com/questions/27972776/having-trouble-implementing-a-readlink-function
#
# Use as follows:
#
#   import junctions
#
#   path = ...
#   if junctions.islink(path):
#     target = junctions.readlink(path)
#
# So if there is a junction C:\tmp\foo with a target of C:\tmp\bar,
# then islink('C:\tmp\foo') returns true, and readlink('C:\tmp\foo')
# returns 'C:\tmp\bar'.
...
def islink(path):
    """ Determines if the path represents a junction. Returns False if path is invalid. """
    result = GetFileAttributesW(path)
    # Check for an invalid file (e.g. path does not exist).
    if result == INVALID_FILE_ATTRIBUTES:
        return False
    # Check if the file attributes indicate a link.
